...
export interface GridLayoutConfig {
  maxVisibleRows?: number;      // default: 12
  rowHeight?: number;           // default: 60
  rowMargin?: number;           // default: 8
  boundaryLinePosition?: number; // default: maxVisibleRows
  repositioningDelayMs?: number; // default: 1000
}
...
const ConfigurableGridLayout: React.FC<ConfigurableGridLayoutProps> = ({ config = {} }) => {
  const {
    maxVisibleRows = 12,
    rowHeight = 60,
    rowMargin = 8,
    boundaryLinePosition,
    repositioningDelayMs = 1000,
  } = config;

  // Calculate boundary line position (defaults to maxVisibleRows if not provided)
  const effectiveBoundaryLinePosition = boundaryLinePosition ?? maxVisibleRows;
  const boundaryPixels = effectiveBoundaryLinePosition * rowHeight + (effectiveBoundaryLinePosition - 1) * rowMargin;
  ...
  const tileExceedsBoundary = useCallback((tile: Layout | GridItem): boolean => {
    ...
    return tileBottomRow > maxVisibleRows;
  }, [maxVisibleRows]);
  ...
  const findAvailableSpace = useCallback((tileToPlace: Layout | GridItem, currentLayout: Layout[]): { x: number; y: number; w: number; h: number } | null => {
    ...
    const grid: boolean[][] = Array(maxVisibleRows).fill(null).map(() => Array(12).fill(false));
    
    // Mark occupied spaces
    currentLayout.forEach(item => {
      if (item.i !== tileToPlace.i) { // Don't consider the tile we're trying to place
        for (let row = item.y; row < item.y + item.h && row < maxVisibleRows; row++) {
    ...
    for (let y = 0; y <= maxVisibleRows - minTileHeight; y++) {
      for (let x = 0; x <= 12 - minTileWidth; x++) {
        ...
        if (canFit) {
          ...
          // Expand height
          for (let h = minTileHeight + 1; h <= maxVisibleRows - y; h++) {
          ...
          return {
            ...
            h: Math.min(maxHeight, tileToPlace.maxH || maxVisibleRows)
          };
        }
      }
    }
  ...
  }, [rollbackToLastValid, removeTile, maxVisibleRows]);
  ...
  }, [tilesExceedingBoundary, handleTileExceedingBoundary, repositioningDelayMs]);
  ...
  const onDrag = useCallback(
    (layout: Layout[], oldItem: Layout, newItem: Layout, placeholder: Layout, e: MouseEvent, element: HTMLElement) => {
      ...
      if (tileBottomRow > maxVisibleRows) {
        // Constrain the y position to keep the tile within boundary
        const maxAllowedY = maxVisibleRows - newItem.h;
  ...
  const onResize = useCallback(
    (layout: Layout[], oldItem: Layout, newItem: Layout, placeholder: Layout, e: MouseEvent, element: HTMLElement) => {
      ...
      if (tileBottomRow > maxVisibleRows) {
        ...
        const maxAllowedHeight = maxVisibleRows - newItem.y;
